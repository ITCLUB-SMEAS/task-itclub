...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        <h2 class="text-lg font-medium text-gray-900 mb-4">Gabung Tim</h2>
        <form action="{{ route('student.teams.join') }}" method="POST" class="flex items-center">
            @csrf
            <div class="flex-1 mr-4">
                <label for="team_code" class="block text-sm font-medium text-gray-700 mb-1">Kode Tim</label>
    ...
    <!-- Tim Saya -->
    <div class="bg-white shadow rounded-lg p-6 mb-6">
        <h2 class="text-lg font-medium text-gray-900 mb-4">Tim Saya</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden shadow-md">
                ...
                <div class="p-4">
                    <p class="text-sm text-gray-600 mb-3">{{ Str::limit($team->description, 100) }}</p>
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="text-xs text-gray-500">Anggota: {{ $team->members->count() }}/{{ $team->max_members }}</p>
                            <p class="text-xs text-gray-500">Ketua: {{ $team->leader->name ?? 'Belum ada' }}</p>
                        </div>
                        <a href="{{ route('student.teams.show', $team->id) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                            Detail →
                        </a>
                    </div>
                </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <h2 class="text-lg font-medium text-gray-900 mb-4">Tim yang Tersedia</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                ...
                <div class="p-4">
                    <p class="text-sm text-gray-600 mb-3">{{ Str::limit($team->description, 80) }}</p>
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="text-xs text-gray-500">Anggota: {{ $team->members->count() }}/{{ $team->max_members }}</p>
                            <p class="text-xs text-gray-500">Ketua: {{ $team->leader->name ?? 'Belum ada' }}</p>
